...
*   Note sur le code mis en oeuvre : l’abstraction autour des compilateurs
...
Le code à prendre en compte se trouve sur la branche **master**. Le code des compilateurs et des tests associés se situe dans le projet *`org.xtext.example.mml.tests`*.
...
4. Se rendre dans le projet *`org.xtext.example.mml`*, dans le dossier *src*/, package *`org.xtext.example.mydsl`*, puis clic droit sur le fichier de grammaire*` Mml.xtext`*, puis en cliquant sur *Run As > Generate Xtext Artifacts*
...
*   Le déport du code commun à tous les compilateurs dans une classe abstraite `AbstractMmlCompiler`
*   La mise en place du PC Façade, pour fournir un jeu de classes du modèle de données plus aisément manipulable que les classes de modèles générées par Xtext, notamment avec : 
    *   utilisation d’énumérations Java plutôt que l’opérateur `instanceof `pour la différenciation des algorithmes, des frameworks, des métriques, …
    *   accès aisé à la liste des features et du label, respectivement grâce aux méthodes `getPredictors()` et `getPredictive(`). La méthode` hasAllPredictors()` indique la sélection de toutes les colonnes.
...
*   d'associer une instance du méta-modèle au  compilateur
...
*   prend en charge l'algorithme spécifié
*   prend en charge les métriques spécifiées
...
Les implémentations des compilateurs pour les différentes cibles se situent dans le package *`core.compilers`* du projet 
*`org.xtext.example.mml.tests`*.
...
C'est à partir de ces fragments de codes que nous écrivons nos compilateurs. Nous nous appuyons sur l’abstraction décrite plus haut pour simplifier ce processus et pour n’avoir qu’à développer la partie génération de code spécifique à la cible.

Pour chaque classe de compilateur, nous créons une classe de test associée qui contient plusieurs cas de tests : ces cas consistent à vérifier que pour un programme MML en entrée, le compilateur produise bien le code attendu. Nous avons choisi de stocker ces programmes attendus dans le dossier _resources/_ du projet, plutôt que de les coder en dur dans le code Java des classes de tests, ceci pour éviter les erreurs liées aux concaténations, à l’échappement de caractères, etc. 
...
Afin de comparer avec un autre jeu de données nous avons utilisé une approximation de la MAPE. Nous avons aussi utilisé le jeu de données "*Titanic*" en cherchant à déterminer l’attribut `survived`. Ici pour chaque algorithme nous avons utilisé la méthode avec les jeux de données séparés en données de test et d’entraînement (30% de test et 70% d’entraînement).
...
GradientBoosting est le plus lent mais est aussi le plus efficace (MSE et MAE plus faibles) alors qu’au contraire le DecisionTree, bien que le plus rapide est aussi le moins fiable (MSE et MAE la plus haute). Le _RandomForest _quand à lui se situe entre les deux en termes de temps et d’efficacité.

On retrouve les mêmes résultats en utilisant le calcul de la MAPE avec les jeux de données _Boston_ et _Titanic_, le classement reste le même en efficacité et en temps d’exécution, même si dans le deuxième cas de figure, les temps d’exécutions pour un même jeu de données restent très proches.
...
_DecisionTree_ est significativement plus rapide et moins précis que les deux autres (d’après les calculs de MSE et de MAE en _cross-validation_).
...
Le jeu de données _Titanic_ est plus long à traiter : pour un même algorithme, en utilisant Scikit-learn et en calculant la MAPE les programmes sont plus longs à s’exécuter avec _Titanic_ qu’avec _Boston_.
...
Il est difficile de statuer étant donné que Scikit-learn est le seul framework que nous avons véritablement réussi à manipuler. Il s’agit d’un framework Python donc la syntaxe est plus compréhensible pour nous que celle du langage R qui est sensiblement différente car il s’agit d’un langage davantage orienté statistiques et mathématiques.
