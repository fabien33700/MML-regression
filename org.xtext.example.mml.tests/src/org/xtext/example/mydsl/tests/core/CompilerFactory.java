...
public class CompilerFactory {
    ...
    private static final List<Class<? extends AbstractMmlCompiler>> availableTargetTypes = Arrays
            .asList(ScikitLearnCompiler.class, RCompiler.class, TensorFlowCompiler.class);
    ...
    public static AbstractMmlCompiler findTargetCompiler(MMLModel model) {
        ...
        for (Class<? extends Compiler<MMLModelFacade>> targetType : availableTargetTypes) {
            try {
                AbstractMmlCompiler compiler = (AbstractMmlCompiler) targetType.getConstructor(MMLModelFacade.class)
        ...
        throw new NoSuchElementException(
                "Could not find a compiler compatible with given model : " + modelFacade.describeTarget());
    }
}
