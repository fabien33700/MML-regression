...
public class ScikitLearnCompiler extends AbstractMmlCompiler {
    ...
    public void writeColumnsExtract() {
        append("df.head()");
        // FIXME très moche mais pas le temps de faire autrement => ajouter valeurs à
        // enlever dans le méta modèle
    ...
    public void writeAlgorithmInvocation() {
        ...
        if (algo == AlgorithmEnum.DT) {
            ...
            Optional.ofNullable(model.getTargetInfo().getSVR_C()).filter(C -> !isBlank(C))
                    .ifPresent(C -> params.put("C", C));

            Optional.ofNullable(model.getTargetInfo().getSVR_SVMKernel()).map(K -> K.getKernel().getLiteral())
                    .filter(K -> !isBlank(K)).map(this::singleQuote).ifPresent(K -> params.put("kernel", K));
        ...
        if (stratification == StratificationEnum.TRAINING_TEST) {
            ...
            append("results = cross_validate(clf, X, y, cv=%d,scoring=scoring)", model.getValidation().getValue());
        }
    }
